export default {
  site: "wechat",
  name: "verify_demand",
  description: "快速验证小程序需求 - 搜索关键词看现有产品评分",
  columns: ["keyword", "result_count", "top_products", "avg_rating", "opportunity"],
  args: {
  ...
  async run(page, args) {
    ...
    try {
      ...
      return [{
        ...
        result_count: 0,
        top_products: "No results or blocked",
        avg_rating: "N/A",
    ...
    const data = await page.eval(() => {
      ...
      for (let i = 0; i < Math.min(articles.length, 10); i++) {
        ...
        const desc = (el.querySelector('.txt-info, .s-p')?.innerText || '').trim()
        
        products.push(`${title} - ${author}`)
      }
    ...
    if (data.count > 100) {
      opportunity = "🔴 红海 - 但可差异化"
    } else if (data.count > 30) {
      opportunity = "🟢 有机会 - 需求被验证"
    } else if (data.count > 0) {
      opportunity = "🟢 蓝海 - 可能是新机会"
    }
