...
Deno.test("[quality/what] Deno has exactly 11 source modules", async () => {
  // Why: detect accidental module sprawl — new modules should be deliberate
  // history.ts added for Meta-Forge trace/version storage
  const sources = await readDenoSources();
  const names = sources.map((s) => s.file).sort();
  assertEquals(
    names,
    ["bridge.ts", "cli.ts", "daemon.ts", "executor.ts", "forge.ts", "history.ts", "inspect.ts", "mcp.ts", "page.ts", "runtime-macos.ts", "runtime-playwright.ts"],
    `Expected 11 modules, got: ${names.join(", ")}`,
  );
});
